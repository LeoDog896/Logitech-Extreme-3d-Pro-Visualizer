<script lang="ts">
  ...
  $: render = ({ context, width, height }: { context: CanvasRenderingContext2D, width: number, height: number }) => {
    const canvasOrigin = {
			x: width / 2,
			y: height / 2
		}
    ...
    context.beginPath();
    context.arc(translatedPosition.x - Math.sin(angle) * 6, translatedPosition.y - Math.cos(angle) * 6, mouseRadius / 4, 0, 2 * Math.PI);
    context.stroke()
  };
</script>
